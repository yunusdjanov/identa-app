...
| Server state | TanStack React Query v5, in-memory per authenticated session |
...
| Media / native | `expo-image-picker`, `expo-updates` (OTA), `expo-haptics` |
...
src/
  ...
  components/  feature folders (appointments, dashboard, patients, payments,
               settings, gallery, treatments) + ui/ primitives
  lib/         hooks + helpers (format, validation, sentry,
...
App.tsx        React Query, providers, Sentry/OTA bootstrap
...
- **Deep linking** (`linking`): prefixes `identa://` and `https://identa.uz`.
  ...
  `patient/:id`, `reset-password` (resolves while
...
profile, team, sessions, analytics, audit, billing, …`). Each mutating
call gates on `requireOnline()` (`lib/offlineGuard.ts`).

Most slices ship an inline **mock branch** for deliberate offline UI demos:
- Master switch `EXPO_PUBLIC_USE_MOCK_API` — only explicit `true` means mocks ON.
- Per-slice overrides `EXPO_PUBLIC_MOCK_{AUTH,PATIENTS,APPOINTMENTS,DASHBOARD,
  PROFILE,TEAM,TREATMENTS,AUDIT}` — explicit `true` enables one slice while
  explicit `false` keeps it real even when the master switch is on.
- **All `.env`/`.env.example` files and all 3 EAS profiles set
  ...
  branch yet: sessions — see `FEATURE_STATUS.md`.)
...
- **React Query** (`App.tsx`): in-memory only; clinical and financial data is never
  dehydrated to AsyncStorage. Defaults: `staleTime 5min`, `gcTime 24h`, `retry 1`,
  refetch on focus/reconnect; mutations `retry: false`. Login, logout, account changes,
  and authorization-scope changes clear the query cache. Startup purges legacy
  `@identa/query-cache-v1..v4` keys. `MutationCache.onError` shows a unified offline toast.
...
- Plugins: expo-font, expo-secure-store, expo-image-picker.
- iOS `infoPlist`: camera / photo-library / FaceID usage strings;
  associatedDomains `identa.uz` + `app.identa.uz`.
- Android permissions include CAMERA and biometric access (location blocked).
...
4. **Never persist protected React Query data** without an explicit encrypted,
   tenant-bound offline-data design and product requirement.
...
- **Maestro** E2E (`.maestro/flows/`) with tags `smoke / auth / navigation /
  treatment` — run via `npm run test:e2e[:smoke|:auth|:nav|…]`.
